...
const ClusterManagement = () => {
  ...
  const [configToDelete, setConfigToDelete] =
    useState<ClusterFallbackConfig | null>(null);
  ...
  const [createFormData, setCreateFormData] =
    useState<ConfigurationFormData>(initialFormData);
  ...
  const handleCreateSubmit = async () => {
    ...
    try {
      ...
      const sourceWorkspace = workspaces.find(
        (w) => w.id === createFormData.sourceWorkspace,
      );
      const destWorkspace = workspaces.find(
        (w) => w.id === createFormData.destinationWorkspace,
      );
      ...
      await clusterFallbackConfigService.create({
        ...
        destination: {
          clusterId:
            destWorkspace?.clusterId ?? createFormData.destinationCluster,
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      // Use workspace.clusterId for consistency - allows backend to use single API call
      const sourceWorkspace = workspaces.find(
        (w) => w.id === editFormData.sourceWorkspace,
      );
      const destWorkspace = workspaces.find(
        (w) => w.id === editFormData.destinationWorkspace,
      );
      ...
      await clusterFallbackConfigService.update(configToEdit.id, {
        ...
        destination: {
          clusterId:
            destWorkspace?.clusterId ?? editFormData.destinationCluster,
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          ...
          <div className="text-muted-foreground text-base">
            Configure and manage cluster fallback settings for your jobs. Each
            source cluster has a designated fallback destination for recovery.
          </div>
        </div>
      ...
      <p className="mt-4 text-sm text-muted-foreground">
        Showing {filteredData.length} of {configurations.length} fallback
        configurations
      </p>
